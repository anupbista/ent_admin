...
const GamesTable = props => {
  ...
  const [mGames, setmGames] = useState(games);
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = games.map((n) => n.id);
  ...
  const handleGameDelete = async () => {
    try {
      ...
      await API.delete('/games/' + selected.join(','), options);
      setSubmitted(true);
      setSelected([])
      toggleLoading(false);
      toggleSnackbar(true);
      toggleSnackbarMsg('Game Deleted')
      props.onClose(true);
    } catch (error) {
      setSubmitted(false);
      toggleLoading(false)
  ...
  useEffect(() => {
    setmGames(games)
    setmGames(games.filter(game => {
      return (
        game.name.toLowerCase().search(searchInput.toLowerCase()) !== -1
      );
    }))
  }, [games,searchInput])
  ...
  return (
    <div className={classes.root}>
      <DeleteDialog deleteModalOpen={deleteModalOpen} confirmDelete={handleGameDelete} onClose={onClose} />
      <FormDialog modalOpen={modalOpen} game={game} onClose={handleDialogClose} isNew={isNew} />
      <GamesToolbar onClose={handleEditClickOpen} onOpen={onSearchChangeHandler} />
            ...
            <TableBody>
              {
                ...
                {
              stableSort(mGames, getComparator(order, orderBy))
