...
const CategoryTable = props => {
  ...
  const [mCategories, setmCategories] = useState(categories);
  ...
  const handleCategoryDelete = async () => {
    try {
      ...
      await API.delete('/category/' + selected.join(','), options);
      setSubmitted(true);
      setSelected([])
      toggleLoading(false);
      toggleSnackbar(true);
  ...
  useEffect(() => {
    ...
    setmCategories(categories.filter(category => {
      return (
  ...
  return (
    <div className={classes.root}>
      <DeleteDialog deleteModalOpen={deleteModalOpen} confirmDelete={handleCategoryDelete} onClose={onClose} />
      <FormDialog modalOpen={modalOpen} category={category} onClose={handleDialogClose} isNew={isNew} />
      <CategoryToolbar onClose={handleEditClickOpen} onOpen={onSearchChangeHandler} />
        ...
        <TableContainer>
          ...
          >
            <EnhancedTableHead
              ...
              rowCount={mCategories.length}
            />
            <TableBody>
              {
                mCategories.length < 1 && (
                  <TableRow>
                ...
                {
              stableSort(mCategories, getComparator(order, orderBy))
