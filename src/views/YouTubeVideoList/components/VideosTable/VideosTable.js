...
const VideosTable = props => {
  ...
  const [mVideos, setmVideos] = useState(videos);
  ...
  const handleVideoDelete = async () => {
    try {
      ...
      await API.delete('/youtubevideos/' + selected.join(','), options);
      setSubmitted(true);
      setSelected([])
      toggleLoading(false);
      toggleSnackbar(true);
      toggleSnackbarMsg('Video Deleted')
      props.onClose(true);
    } catch (error) {
      setSubmitted(false);
      toggleLoading(false)
  ...
  useEffect(() => {
    setmVideos(videos)
    setmVideos(videos.filter(video => {
      return (
        video.name.toLowerCase().search(searchInput.toLowerCase()) !== -1
      );
    }))
  }, [videos,searchInput])
  ...
  return (
    <div className={classes.root}>
      <DeleteDialog deleteModalOpen={deleteModalOpen} confirmDelete={handleVideoDelete} onClose={onClose} />
      <FormDialog modalOpen={modalOpen} video={video} onClose={handleDialogClose} isNew={isNew} />
      <VideosToolbar onClose={handleEditClickOpen} onOpen={onSearchChangeHandler} />
            ...
            <TableBody>
              {
                ...
                {
              stableSort(mVideos, getComparator(order, orderBy))
